...
const SetDailyMenu = () => {
	...
	const loadOptions = (inputValue, callback) => {
		Api.get(`meals?search=${inputValue}`).then((response) => {
	...
	const handleSetDailyMealMenu = () => {
		...
		if (meals.length > 0) {
			Api.post('menus', { meal_id: meals }).then((response) => {
	...
	const renderNoMealSelectedContent = () => {
		return (
			<div
				className={
					'wm-card wm-bg-gray-100 wm-mt-4 wm-h-32 wm-flex wm-items-center wm-justify-center'
				}
			>
				<div className={'wm-flex wm-items-center'}>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						className="wm-h-5 wm-w-5 wm-text-gray-500"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
					...
					<p className={'wm-text-sm wm-text-gray-600 wm-ml-1'}>
						{__('Select meals to set daily menu.', 'we-meal')}
					</p>
				</div>
			</div>
		);
	...
	return (
		<div className={'wm-mt-4'}>
			<h1 className={'wm-text-gray-600 wm-text-sm wm-font-medium'}>
				{__('Search Meal:', 'we-meal')}
			</h1>
